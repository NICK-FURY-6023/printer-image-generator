    ...
    <div ref={cardRef} className={`label-card${isActive ? ' active' : ''} animate-fade-in`}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {filled && (
            <span style={{ fontSize: 10, color: '#22c55e', background: 'rgba(34,197,94,0.1)', padding: '2px 6px', borderRadius: 10 }}>
              ✓ filled
            </span>
          )}
      ...
      {open && (
        <div style={{ padding: '14px', background: '#0f172a', borderTop: '1px solid #1e293b' }}>
  ...
  return (
    <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setApplyPanelOpen(o => !o)} className="btn-ghost" style={{ fontSize: 11 }}>
          {applyPanelOpen ? '▲' : '▼'} Bulk Fill
        </button>
      </div>
      ...
      <div style={{ background: '#1e293b', borderRadius: 12, padding: '12px 14px', border: '1px solid #334155' }}>
        ...
        <div style={{ display: 'flex', gap: 14, marginTop: 10, fontSize: 10, color: '#475569' }}>
          <span><span style={{ color: '#f97316' }}>●</span> Active</span>
          <span><span style={{ color: '#22c55e' }}>●</span> Filled</span>
          <span><span style={{ color: '#334155' }}>●</span> Empty</span>
        </div>
      </div>
      ...
      {applyPanelOpen && (
        <div className="animate-fade-in" style={{
          background: 'rgba(249,115,22,0.06)', border: '1px solid rgba(249,115,22,0.2)', borderRadius: 12, padding: '14px',
        }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#f97316', letterSpacing: '0.08em', marginBottom: 12 }}>
            ⚡ APPLY TO ALL LABELS
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          ...
          <button onClick={handleApplyAll} className="btn-saffron" style={{ width: '100%', justifyContent: 'center', marginTop: 12, padding: '10px' }}>
